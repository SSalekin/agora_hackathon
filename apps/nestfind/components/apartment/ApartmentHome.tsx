    ...
    <div className="min-h-dvh bg-background pb-24 text-foreground sm:pb-0">
      <header className="sticky top-0 z-30 border-b border-border/80 bg-background/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          <button type="button" onClick={() => setTab('discover')} className="flex items-center gap-2" aria-label="NestFind home"><span className="grid h-9 w-9 place-items-center rounded-2xl bg-primary text-white"><Home className="h-4 w-4" /></span><div className="leading-tight"><span className="block font-serif text-lg font-bold sm:text-xl">NestFind</span><span className="hidden text-[11px] text-muted-foreground sm:block">{resultSummary}</span></div></button>
          <nav className="hidden items-center gap-1 rounded-full border border-border bg-background p-1 sm:flex" aria-label="Main navigation">
            {([['discover', 'Discover', Search], ['saved', `Saved ${favoriteIds.length ? `(${favoriteIds.length})` : ''}`, Heart], ['history', 'History', Clock3], ...(user?.role === 'landlord' ? [['landlord', 'Landlord', Home] as const] : []), ...(user?.role === 'moderator' ? [['moderator', 'Moderator', Shield] as const] : [])] as const).map(([value, label, Icon]) => <button type="button" key={value} onClick={() => setTab(value)} className={`flex items-center gap-1.5 rounded-full px-4 py-2 text-xs font-semibold ${tab === value ? 'bg-primary text-white' : 'text-muted-foreground hover:text-foreground'}`}><Icon className="h-3.5 w-3.5" />{label}</button>)}
          </nav>
          <div className="flex items-center gap-2"><div className="hidden sm:block"><PwaControls /></div>{userName ? <div className="flex items-center gap-2 rounded-full border border-border bg-background py-1 pl-1 pr-2 text-xs font-semibold"><span className="grid h-7 w-7 place-items-center rounded-full bg-amber-100">{userName[0].toUpperCase()}</span><span className="hidden md:block">{userName}</span><button type="button" onClick={handleSignOut} aria-label="Sign out"><LogOut className="h-3.5 w-3.5" /></button></div> : <button type="button" onClick={openSignIn} className="flex h-9 items-center gap-2 rounded-full bg-primary px-3 text-xs font-semibold text-white sm:px-4"><UserRound className="h-4 w-4" /> <span className="hidden sm:inline">Sign in</span></button>}</div>
        </div>
      </header>

      {tab === 'discover' && <main>
        <section className="relative overflow-hidden border-b border-border"><div className="absolute inset-0 opacity-70 [background-image:radial-gradient(circle_at_15%_20%,#d9ead3_0,transparent_28%),radial-gradient(circle_at_85%_35%,#f8dfb6_0,transparent_25%)]" /><div className="absolute inset-x-0 top-0 h-full soft-grid opacity-40" /><div className="relative mx-auto grid max-w-7xl gap-6 px-4 py-8 sm:px-6 sm:py-12 lg:grid-cols-[1.1fr_.9fr] lg:gap-10 lg:py-16">
          <div><div className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-3 py-1.5 text-xs font-semibold text-primary"><Sparkles className="h-3.5 w-3.5" /> Voice-powered apartment discovery</div><h1 className="mt-5 max-w-3xl font-serif text-4xl font-bold leading-[1.02] tracking-[-0.04em] sm:text-5xl lg:text-6xl">Find a place that feels like <span className="italic text-primary">home.</span></h1><p className="mt-4 max-w-xl text-sm leading-6 text-muted-foreground sm:text-base sm:leading-7">Tell our voice concierge where, when, and how much. NestFind turns the conversation into apartment matches around Da Nang.</p>
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center"><button type="button" onClick={userName ? onStartConversation : openSignIn} disabled={isLoading} className="group flex h-13 items-center justify-center gap-3 rounded-2xl bg-primary px-5 text-sm font-semibold text-white shadow-lg transition hover:bg-primary/90 disabled:opacity-60 sm:h-14 sm:px-6"><span className="grid h-8 w-8 place-items-center rounded-full bg-white/15"><Mic className="h-4 w-4" /></span>{isLoading ? 'Connecting…' : 'Talk to your apartment concierge'}<ArrowRight className="h-4 w-4" /></button><p className="text-xs leading-5 text-muted-foreground sm:max-w-[14rem]">Voice works best for budgets, dates, and neighborhoods in one sentence.</p></div>{error && <p className="mt-3 text-sm text-destructive">{error}</p>}
          </div>
