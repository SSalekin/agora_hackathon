...
type Props = {
  ...
  listings: ApartmentListing[]; listingCatalog: ApartmentListing[]; hasSearched: boolean; activeFilters: ListingSearchFilters | null; favoriteIds: string[]; history: SearchHistoryItem[]; isListingSearchLoading: boolean;
...
export function ApartmentHome({ isLoading, error, userName, listings, listingCatalog, hasSearched, activeFilters, favoriteIds, history, isListingSearchLoading, onStartConversation, onTextSearch, onToggleFavorite }: Props) {
  ...
  return (
    <div className="min-h-dvh bg-white pb-24 text-stone-900 sm:pb-0">
      <header className="sticky top-0 z-30 border-b border-stone-200/80 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          <button type="button" onClick={() => setTab('discover')} className="flex items-center gap-2" aria-label="NestFind home"><span className="grid h-9 w-9 place-items-center rounded-2xl bg-blue-600 text-white"><Home className="h-4 w-4" /></span><div className="leading-tight"><span className="block font-serif text-lg font-bold sm:text-xl">NestFind</span><span className="hidden text-[11px] text-stone-500 sm:block">{resultSummary}</span></div></button>
