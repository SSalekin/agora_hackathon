...
function ListingCard({ listing, isFavorite, onToggleFavorite, onOpenAgreement }: {
  ...
  return (
    <article className="group flex h-full min-h-[23rem] flex-col overflow-hidden rounded-3xl border border-border bg-background shadow-[0_14px_40px_rgba(58,45,30,0.08)] transition hover:-translate-y-1 hover:shadow-[0_20px_50px_rgba(58,45,30,0.14)] md:min-h-[26rem]">
      <div className={`relative h-16 bg-gradient-to-br md:h-36 ${listing.accent}`}>
        ...
        <span className="absolute left-2 top-2 rounded-full bg-background/85 px-3 py-1 text-xs font-semibold text-foreground backdrop-blur md:left-3 md:top-3">{listing.distanceKm} km away</span>
        <button type="button" onClick={() => onToggleFavorite(listing.id)} className="absolute right-2 top-2 grid h-10 w-10 place-items-center rounded-full bg-background/85 text-muted-foreground shadow-sm backdrop-blur transition hover:scale-105 hover:text-destructive md:right-3 md:top-3" aria-label={isFavorite ? 'Remove from saved listings' : 'Save listing'}>
      ...
      <div className="flex flex-1 flex-col p-3 pt-2 md:p-5">
        <div className="flex min-h-[5.5rem] items-start justify-between gap-3 md:min-h-[6.5rem]">
          ...
          <div className="shrink-0 text-right"><p className="text-base font-bold text-primary md:text-lg">{formatVnd(listing.monthlyRentVnd)}</p><p className="text-[11px] text-muted-foreground">per month</p></div>
        </div>
        <div className="flex-1">
          ...
          <div className="mt-2 grid grid-cols-3 gap-2 border-y border-stone-100 py-2 text-xs font-medium text-muted-foreground md:mt-4 md:py-3">
          ...
          <div className="mt-2 flex flex-wrap gap-2 md:mt-4">{listing.amenities.slice(0, 2).map((amenity) => <span key={amenity} className="rounded-full bg-muted px-2.5 py-1 text-[11px] text-muted-foreground">{amenity}</span>)}{listing.amenities.length > 2 && <span className="rounded-full bg-primary/10 px-2.5 py-1 text-[11px] font-medium text-primary">+{listing.amenities.length - 2} more</span>}</div>
        </div>
        <div className="mt-2 md:mt-4">
          <button type="button" onClick={() => onOpenAgreement(listing.id)} className="w-full rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white md:py-3">Start rental agreement</button>
        </div>
      </div>
    </article>
  );
...
export function ListingGrid({ listings, favoriteIds, onToggleFavorite, emptyMessage = 'No apartments match those filters yet.' }: ListingGridProps) {
  ...
  if (listings.length === 0) {
    return <div className="rounded-3xl border border-dashed border-stone-300 bg-background/60 px-6 py-14 text-center text-sm text-muted-foreground">{emptyMessage}</div>;
  }
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-4">
            <div className="mx-auto max-h-[90dvh] w-full max-w-2xl overflow-y-auto rounded-t-[1.75rem] bg-background p-5 shadow-xl sm:rounded-2xl sm:p-6">
              <div className="flex items-start justify-between">
                ...
                <button type="button" onClick={() => setActiveListingId(null)} className="text-muted-foreground">Close</button>
              </div>
              <div className="mt-4">
                {TenantPanel ? <TenantPanel listing={listing} onClose={() => setActiveListingId(null)} /> : <div className="py-8 text-center text-sm text-muted-foreground">Loading agreement panel…</div>}
              </div>
            </div>
          </div>
        );
