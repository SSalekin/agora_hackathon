...
This guide is for coding agents making changes in `nestfind`.

## How to Load

The `docs/ai/` directory is not present in this repository. Use the sections below (Start Here, Current System Shape, Key Files, Patterns, etc.) as the canonical reference.

## Start Here

- Read [README.md](./README.md) for setup, commands, verification, and deployment.
- For layout and responsibilities inside `components/`, `app/api/`, and `lib/`, review the Key Files section below.
...
- If a workflow, request contract, or ownership boundary changes, update `README.md` and `AGENTS.md` in the same change.
...
- `app/api/listings/route.ts`: typed apartment search and catalog endpoint backed by Couchbase when configured.
...
- `lib/listings.ts`: local seed fixture and deterministic filter/search functions.
- `lib/apartment-catalog.ts`: selects Couchbase or the unconfigured local fallback.
- `lib/db/apartment-listings.ts`: Couchbase catalog document reads and writes.
- `scripts/seed-couchbase-listings.ts`: idempotent Couchbase demo-data migration.
...
3. If you changed files in `components/` or `app/api/`, verify that `README.md` and this file still match the implementation.
4. Update root README when workflow, request contracts, architecture, or environment guidance changes.
