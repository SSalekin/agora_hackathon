...
- [ ] Decode Anchor enum state objects into stable names such as `awaitingLandlordApproval`, `awaitingFunding`, and `funded`. The current `.toString()` result does not satisfy the button-state comparisons.
- [ ] Require and validate an explicit landlord public key. The current fallback uses the tenant wallet as landlord, but the program correctly rejects duplicate roles.

## PWA escrow integration

### Wallet foundation — partially implemented
...
- [ ] Handle wallet disconnect, account changes, wrong network, unavailable wallet, rejected signatures, and insufficient balance.
- [ ] Decide how browser RPC access is provided without exposing an unrestricted provider API key.

### Listing and agreement terms

- [ ] Add the landlord wallet address to each local and Couchbase listing.
- [ ] Add the landlord-proposed deposit amount and default inspection period/deadline to listing or agreement-term data.
- [ ] Stop asking tenants to type trusted landlord/deposit values when those values should come from the selected listing.
...
- [ ] Implement on-chain `cancel_agreement`; the current button only changes local React state.
- [ ] Implement on-chain `release_by_tenant`; the current button only changes local React state.
- [ ] Implement on-chain `open_dispute` with a non-zero reason code and a 32-byte evidence hash; the current action is local-only.
- [ ] Load an existing agreement PDA after refresh instead of keeping the agreement only in component state.

### Landlord actions

- [ ] Add a landlord dashboard or agreement view.
- [ ] Discover agreements assigned to the connected landlord.
- [ ] Display the exact tenant, listing hash, deposit amount, and deadline before approval.
- [ ] Implement on-chain `approve_agreement`.
- [ ] Implement landlord rejection/pre-funding cancellation through `cancel_agreement`.
- [ ] Implement on-chain `release_after_deadline` with a clear countdown and early-release error state.

### Moderator actions

- [ ] Add a moderator-only dispute queue.
- [ ] Load the off-chain reason/evidence record and verify its hash against the agreement account.
- [ ] Implement `resolve_dispute(true)` for full release to the landlord.
- [ ] Implement `resolve_dispute(false)` for full refund to the tenant.
- [ ] Clearly disclose that the hackathon moderator is centralized and decisions are final.
